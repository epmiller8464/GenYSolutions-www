(function () {
    ...
    App = Em.Application.create({
        ready: function () {
            $("textarea.steppable,input.steppable[type=text]").on('keydown mousewheel', function (e) {
            ...
            $(window).resize(function () {
                var _h = $("#hero").innerHeight();
                var _w = $("#hero").innerWidth();
                console.log("h,w", _h, _w);
                App.graph.resize(_h, _w);
    ...
    App.lcfCodeObject = Ember.Object.extend({
        ...
        parse: function () {
            ...
            if (match[1] != "") {
                this.steps = match[1].split(',').filter(Number).map(function (e) {
                    return +e
                });
            } else {
    ...
    App.graph = Ember.Object.create({
        init: function () {
            this._super(),
                this.width = $("#hero").innerWidth(),
                this.height = $("#hero").innerHeight(),
            ...
            }).size([this.rcx/*- (this.rcx * 0.25)*//* this.rcx * 2 */, this.height]);//offsets center radius
        ...
        resize: function (height, width) {
            ...
            this.force.size([this.rcx, height]);
            this.redraw()
        },
        prevFrame: function () {
        ...
        draw: function (lcfCodeObject) {
            ...
            //todo:swap with other project
            var rx = Math.random(),
                rz = Math.random();
            var _x = ((this.width / 2 ) * rx) >> 0;
            var _y = ((this.height / 2 ) * rz) >> 0;
            ...
            if (this.get('animationSpeed') == 0) {
                ...
                this.nodes.slice(1).forEach(function (target, i) {
                    App.graph.links.push({
                        source: App.graph.nodes[i == App.graph.nodes.length - 2 ? 1 : i + 2],
                        target: target,
                        linkDistance: 0
                    })
                });
                ...
                this.drawLines();
                var _interval = this.get('animationSpeed');
                this.interval = setTimeout(this.animateLcf, _interval);
            }
        ...
        animateLcf: function () {
            ...
            if (graph.currentVertexOffset == 0) {
                circles.filter(function (d, i) {
                    return graph.currentVertex == i
                }).style("fill", graph.colors[2]);
                circles.filter(function (d, i) {
                    return graph.currentVertex > i
                }).style("fill", graph.colors[0]);
                circles.filter(function (d, i) {
                    return graph.currentVertex < i
                }).style("fill", "white");
                ...
                circles.filter(function (d, i) {
                    return graph.currentStep == i
                }).style("fill", graph.colors[1]);
                graph.currentVertexOffset += (stepInstruction > 0 ? 1 : -1)
            } else {
                circles.filter(function (d, i) {
                    return graph.currentStep == i
                ...
                graph.links.push({
                    source: graph.nodes.slice(1)[graph.currentVertex],
                    target: graph.nodes.slice(1)[graph.currentStep]
                });
                graph.drawLines();
                graph.currentVertex++;
            ...
            if (graph.currentVertex != numVertices) {
                graph.interval = setTimeout(function () {
                    App.graph.animateLcf()
                }, App.graph.get('animationSpeed'));
            } else {
                graph.interval = null;
        ...
        drawLines: function () {
            var lines = this.svg.selectAll("line.link").data(this.links);
    ...
    App.lcfCodesController = Ember.ArrayController.create({
        content: Ember.A([
            ...
            },
            {
                code: '[3]6',
                name: 'Utility graph'
            }, {code: '[3,-3]4', name: 'Cubical graph'}, {code: '[4]8', name: 'Wagner graph'}, {
                code: '[6,4,-4]4',
                name: 'Bidiakis cube'
            }, {code: '[5,-5]6', name: 'Franklin graph'}, {
                code: '[-5,-2,-4,2,5,-2,2,5,-2,-5,4,2]',
                name: 'Frucht graph'
                ...
                //code: '[10,7,4,-4,-7,10,-4,7,-7,4]2',
                code: '[69,73,-97,65,-60,81,35,-55,95,19,-52,2,-26,-6,57,-91,83,-63,-9,-12,47,28,33,-23,-93,-44,-84,60,-11,4,67,-5,-79,16,84,51]25',
                name: 'Dodecahedral graph'
            }, {code: '[12,7,-7]8', name: 'McGee graph'}, {
                code: '[2,9,-2,2,-9,-2]4',
                name: 'Truncated cubical graph'
            }, {
                //code: '[3,-7,7,-3,9,-21]18',
                code: '[69,73,-97,65,-60,81,35,-55,95,19,-52,2,-26,-6,57,-91,83,-63,-9,-12,47,28,33,-23,-93,-44,-84,60,-11,4,67,-5,-79,16,84,51]25',
                ...
                code: '[-13,-9,7,-7,9,13]5',
                name: 'Tutte–Coxeter graph'
            }, {code: '[5,-5,13,-13]8', name: 'Dyck graph'}, {
                code: '[-25,7,-7,13,-13,25]9',
                name: 'Gray graph'
            }, {
                code: '[30,-2,2,21,-2,2,12,-2,2,-12,-2,2,-21,-2,2,30,-2,2,-12,-2,2,21,-2,2,-21,-2,2,12,-2,2]4',
                //code: '[9,25,31,-17,17,33,9,-29,-15,-9,9,25,-25,29,17,-9,9,-27,35,-9,9,-17,21,27,-29,-9,-25,13,19,-9,-33,-17,19,-31,27,11,-25,29,-33,13,-13,21,-29,-21,25,9,-11,-19,29,9,-27,-19,-13,-35,-9,9,17,25,-9,9,27,-27,-21,15,-9,29,-29,33,-9,-25]',
                name: 'Truncated dodecahedral graph'
            }, {
                code: '[-29,-19,-13,13,21,-27,27,33,-13,13,19,-21,-33,29]5',
                name: 'Harries graph'
            }, {
                code: '[9,25,31,-17,17,33,9,-29,-15,-9,9,25,-25,29,17,-9,9,-27,35,-9,9,-17,21,27,-29,-9,-25,13,19,-9,-33,-17,19,-31,27,11,-25,29,-33,13,-13,21,-29,-21,25,9,-11,-19,29,9,-27,-19,-13,-35,-9,9,17,25,-9,9,27,-27,-21,15,-9,29,-29,33,-9,-25]',
                name: 'Harries–Wong graph'
            }, {
                code: '[-9,-25,-19,29,13,35,-13,-29,19,25,9,-29,29,17,33,21,9,-13,-31,-9,25,17,9,-31,27,-9,17,-19,-29,27,-17,-9,-29,33,-25,25,-21,17,-17,29,35,-29,17,-17,21,-25,25,-33,29,9,17,-27,29,19,-17,9,-27,31,-9,-17,-25,9,31,13,-9,-21,-33,-17,-29,29]',
                name: 'Balaban 10-cage'
            }, {
                code: '[17,-9,37,-37,9,-17]15',
                name: 'Foster graph'
            }, {
                code: '[16,24,-38,17,34,48,-19,41,-35,47,-20,34,-36,21,14,48,-16,-36,-43,28,-17,21,29,-43,46,-24,28,-38,-14,-50,-45,21,8,27,-21,20,-37,39,-34,-44,-8,38,-21,25,15,-34,18,-28,-41,36,8,-29,-21,-48,-28,-20,-47,14,-8,-15,-27,38,24,-48,-18,25,38,31,-25,24,-46,-14,28,11,21,35,-39,43,36,-38,14,50,43,36,-11,-36,-24,45,8,19,-25,38,20,-24,-14,-21,-8,44,-31,-38,-28,37]',
                ...
                code: '[47,-23,-31,39,25,-21,-31,-41,25,15,29,-41,-19,15,-49,33,39,-35,-21,17,-33,49,41,31,-15,-29,41,31,-15,-25,21,31,-51,-25,23,9,-17,51,35,-29,21,-51,-39,33,-9,-51,51,-47,-33,19,51,-21,29,21,-31,-39]2',
                name: 'Ljubljana graph'
            }, {code: '[17,27,-13,-59,-35,35,-11,13,-53,53,-27,21,57,11,-21,-57,59,-17]7', name: 'Tutte 12-cage'}]),
